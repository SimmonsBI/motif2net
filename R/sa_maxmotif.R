sa_maxmotif <- function(mat_initial, target_motif, six_node, steps = 1000, iterations_at_each_temp = 1, alpha = 0.01, available_mutations, connectance){
  # initialise matrices
  mat_optimum <- mat_initial
  mat_current <- mat_initial
  mat_neighbour <-  mat_initial
  # initialise costs
  cost_initial <- cost_maxmotif(target_motif = target_motif, solution = mat_initial, six_node = six_node)
  ...
  for(i in 1:steps){
    for(j in 1:iterations_at_each_temp){
      ...
      cost_neighbour <- cost_maxmotif(target_motif = target_motif, solution = mat_neighbour, six_node = six_node)
